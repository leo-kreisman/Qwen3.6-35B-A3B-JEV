...
def compare(reference,candidate):
    ...
    if len(base)!=len(reference['answers']) or len(other)!=len(candidate['answers']):raise ValueError('duplicate decision IDs')
    if base.keys()!=other.keys():raise ValueError('different decision sets')
    flips=[];maximum=0.;logit_max=0.
    for key,a in base.items():
        b=other[key]
        if not a['probabilities'] or len({len(a['probabilities']),len(b['probabilities']),len(a['logits']),len(b['logits'])})!=1:raise ValueError('different options/logits')
        if not all(math.isfinite(v) for r in (a,b) for field in ('probabilities','logits') for v in r[field]):raise ValueError('nonfinite result')
