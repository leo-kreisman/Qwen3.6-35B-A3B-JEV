...
class NativeTiles {
    ...
    bool overlap_io;
    ggml_context * activation_ctx=nullptr;
    ggml_tensor * activation_g=nullptr,*activation_u=nullptr,*activation_h=nullptr;
    ggml_cgraph * activation_graph=nullptr;
    std::vector<uint8_t> activation_work;
    ...
    void read(const Block & b) {
        ...
        do {got=pread(fd,buffer,count,b.offset);}while(got<0&&errno==EINTR);
        double elapsed=std::chrono::duration<double>(std::chrono::steady_clock::now()-start).count();
        io_seconds+=elapsed;io_wait_seconds+=elapsed;
    ...
    void activate(float * gate,float * up,float * result,int width,int rows) {
        ...
        if(!activation_ctx) {
            activation_ctx=ggml_init(ip);if(!activation_ctx)throw std::runtime_error("activation graph allocation failed");
            activation_g=ggml_new_tensor_2d(activation_ctx,GGML_TYPE_F32,width,rows);
            activation_u=ggml_new_tensor_2d(activation_ctx,GGML_TYPE_F32,width,rows);
            activation_h=ggml_swiglu_split(activation_ctx,activation_g,activation_u);
            activation_graph=ggml_new_graph_custom(activation_ctx,8,false);
            ggml_build_forward_expand(activation_graph,activation_h);++activation_graph_builds;
        }
        // This graph contains only a contiguous elementwise SwiGLU. Update all
        // three shapes together and re-plan; never reuse a shape-dependent plan.
        for(auto * t:{activation_g,activation_u,activation_h}) {
            if(t->ne[0]!=width)throw std::runtime_error("activation width changed");
            t->ne[1]=rows;t->nb[2]=t->nb[1]*rows;t->nb[3]=t->nb[2];
        }
        activation_g->data=gate;activation_u->data=up;activation_h->data=result;
        auto plan=ggml_graph_plan(activation_graph,1,nullptr);
        if(ip.mem_size+plan.work_size>(1ULL<<20))throw std::runtime_error("activation scratch exceeds reserved budget");
        activation_work.resize(plan.work_size);plan.work_data=activation_work.data();
        auto status=ggml_graph_compute(activation_graph,&plan);++activation_calls;
...
public:
    ...
    uint64_t activation_graph_builds=0,activation_calls=0,input_quantized_rows=0;
    size_t peak_work_bytes=0;
    double compute_seconds=0;
    double io_seconds=0,io_wait_seconds=0;
    explicit NativeTiles(const std::string & directory,int nthreads=6,bool staged_down=true,bool overlap=false):threads(nthreads),preserve_reduction(staged_down),overlap_io(overlap) {
    ...
    ~NativeTiles() { if(fd>=0)close(fd);free(buffer);if(activation_ctx)ggml_free(activation_ctx); }
    std::vector<float> evaluate(const std::vector<float> & x,const std::vector<int32_t> & ids,int n,int k) {
        ...
        const bool shared_input=ggml_get_type_traits_cpu(types[0])->vec_dot_type==ggml_get_type_traits_cpu(types[1])->vec_dot_type;
        ...
        // Extra buffer, immutable read descriptors, and a conservative worker
        // stack/runtime allowance are included before starting asynchronous I/O.
        if(overlap_io)work+=io+2*size_t(experts)*(m/tile)*sizeof(NativeTileIO::Read)+(16ULL<<20);
        ...
        std::unique_ptr<NativeTileIO> pipeline;
        if(overlap_io) {
            std::vector<NativeTileIO::Read> jobs;
            for(int e=0;e<experts;++e)if(!ready[e].empty())for(const auto & b:blocks[e]) {
                size_t payload=b.length[0]+b.length[1]+b.length[2];
                jobs.push_back({b.offset,payload,(payload+4095)/4096*4096});
            }
            pipeline=std::make_unique<NativeTileIO>(fd,std::move(jobs),io);
        }
        size_t consumed_tiles=0;
        std::vector<uint8_t> qx[2];
        for(int t=0;t<(shared_input?1:2);++t) {
            ...
            input_quantized_rows+=n;
        }
        const auto & up_input=qx[shared_input?0:1];
        std::vector<float> output(size_t(n)*k*d,0);
        size_t max_rows=0;for(const auto & rows:ready)max_rows=std::max(max_rows,rows.size());
        // Allocate once per admitted group, rather than once per expert. These
        // buffers cannot outlive this invocation, so large groups retain no heap.
        std::vector<float> hidden(max_rows*tile),gates(max_rows*tile),ups(max_rows*tile);
        std::vector<uint8_t> qhidden(preserve_reduction?0:max_rows*qhbytes);
        size_t down_full_stride=row_bytes(types[2],m);
        size_t qfull_stride=row_bytes(ggml_get_type_traits_cpu(types[2])->vec_dot_type,m);
        std::vector<float> full_hidden(preserve_reduction?max_rows*m:0);
        std::vector<uint8_t> full_down(preserve_reduction?d*down_full_stride:0);
        std::vector<uint8_t> qfull(preserve_reduction?max_rows*qfull_stride:0);
        for(int e=0;e<experts;++e) {
            const auto & rows=ready[e];if(rows.empty())continue;
            ++expert_visits;routed_rows+=rows.size();
            for(const auto & b:blocks[e]) {
                const void * data;
                if(pipeline)data=pipeline->acquire(consumed_tiles);
                else {read(b);data=buffer;}
                auto * gate=static_cast<const uint8_t*>(data);
                ...
                for(size_t r=0;r<rows.size();++r) {
                    ...
                    for(int j=0;j<tile;++j) {
                        ...
                        float u=dot(types[1],d,up+j*up_stride,up_input.data()+input_row*qxbytes[1]);
                ...
                if(pipeline)pipeline->release(consumed_tiles++);
            }
            if(preserve_reduction) {
                #pragma omp parallel for num_threads(threads)
                for(size_t r=0;r<rows.size();++r) {
        ...
        if(pipeline)pipeline->finish(read_bytes,read_calls,io_seconds,io_wait_seconds);
    ...
    json stats() const { return {{"direct_read_bytes",read_bytes},{"read_calls",read_calls},
        {"overlap_io",overlap_io},{"io_service_seconds",io_seconds},{"io_wait_seconds",io_wait_seconds},
        {"activation_graph_builds",activation_graph_builds},{"activation_calls",activation_calls},{"input_quantized_rows",input_quantized_rows},
